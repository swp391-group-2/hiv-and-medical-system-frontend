...
const MarkdownRenderer: React.FC<MarkdownRendererProps> = ({ 
  ...
  const renderMarkdown = (markdown: string): string => {
    ...
    html = html.replace(/```([\s\S]*?)```/g, '<pre class="bg-gray-100 p-4 rounded-lg overflow-x-auto my-4 border"><code class="text-sm font-mono text-gray-800">$1</code></pre>');

    // Code inline
    html = html.replace(/`([^`]+)`/g, '<code class="bg-gray-100 px-2 py-1 rounded text-sm font-mono text-red-600 border">$1</code>');
    ...
    html = html.replace(/^> (.*$)/gim, '<blockquote class="border-l-4 border-blue-500 pl-4 py-2 my-4 bg-blue-50 text-gray-700 italic rounded-r">$1</blockquote>');

    // Lists với styling tốt hơn
    html = html.replace(/^- (.*$)/gim, '<li class="ml-6 mb-1 relative"><span class="absolute -ml-4 text-blue-600">•</span>$1</li>');
    html = html.replace(/^(\d+)\. (.*$)/gim, '<li class="ml-6 mb-1 relative list-decimal list-inside">$2</li>');
