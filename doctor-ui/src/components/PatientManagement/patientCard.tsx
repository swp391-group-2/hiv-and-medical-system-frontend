...
import BasicModal from "@/components/Modal/basicModal"; // Đảm bảo path đúng
        ...
        <Tabs value={tab} onValueChange={setTab} className="w-full">
          ...
          <TabsContent value="info">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm text-gray-700">
              ...
              <div><b>Mã BN:</b> {patient.patientId}</div>
              <div><b>Email:</b> {patient.email || "Không rõ"}</div>
              <div><b>Giới tính:</b> {patient.gender || "Không rõ"}</div>
              <div><b>Ngày sinh:</b> {patient.dob || "Không rõ"}</div>
              <div><b>Địa chỉ:</b> {patient.address || "Không rõ"}</div>
              <div><b>SĐT:</b> {patient.phoneNumber || "Không rõ"}</div>
              <div><b>CMND:</b> {patient.identificationCard || "Không rõ"}</div>
              <div><b>Bảo hiểm:</b> {patient.healthInsurance || "Không rõ"}</div>
              <div><b>Nghề nghiệp:</b> {patient.occupation || "Không rõ"}</div>
              <div><b>Trạng thái:</b> {patient.userStatus || "Không rõ"}</div>
            </div>
          </TabsContent>
          ...
          <TabsContent value="protocol">
            {prescription ? (
              <div className="space-y-3 text-sm text-gray-700">
                <div><b>Tên phác đồ:</b> {prescription.name}</div>
                <div><b>Mô tả:</b> {prescription.description || "Không rõ"}</div>
                <div><b>Chống chỉ định:</b> {prescription.contraindication || "Không rõ"}</div>
                <div><b>Hướng dẫn:</b> {prescription.instructions || "Không rõ"}</div>
              </div>
            ) : (
